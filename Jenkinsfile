      ...
      parallel {       
        stage('BERT offline preprocessing') {
          steps {
            ...
            sh 'cd examples/nlp/language_modeling && LOSS=$(cat outputs/bert_lm/log_globalrank-0_localrank-0.txt |  grep "Loss" |tail -n 1| awk "{print $7}" | egrep -o "[0-9.]+" ) && echo $LOSS && if [ $(echo "$LOSS > 13.0" | bc -l) -eq 1 ]; then echo "FAILURE" && exit 1; else echo "SUCCESS"; fi'
